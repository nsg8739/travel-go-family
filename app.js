...
$('#phpAmount').oninput=e=>{const value=Number(e.target.value);$('#krwAmount').value=e.target.value===''?'':Math.round(value*phpRate)};
$('#krwAmount').oninput=e=>{const value=Number(e.target.value);$('#phpAmount').value=e.target.value===''?'':Number((value/phpRate).toFixed(2))};window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();installPrompt=e;$('#installBtn').hidden=false});
