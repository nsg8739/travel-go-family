...
function renderShortcuts(){const el=$('#shortcutList');el.innerHTML='';const items=data.shortcuts||[];if(!items.length)return empty(el,'여행 중 자주 사용하는 앱이나 사이트를 등록해 보세요.');items.forEach(x=>{const isGrab=/그랩|grab/i.test(x.name||'');const grabUrl=/Android/i.test(navigator.userAgent)?'intent://open?screenType=BOOKING#Intent;scheme=grab;package=com.grabtaxi.passenger;action=android.intent.action.VIEW;category=android.intent.category.BROWSABLE;end':'grab://open';const safeUrl=isGrab?grabUrl:safeShortcutUrl(x.url);el.insertAdjacentHTML('beforeend',`<article class="shortcut-card"><div class="shortcut-actions"><button class="edit" data-edit="shortcut" data-id="${x.id}" aria-label="바로가기 수정">✎</button><button class="delete" data-del="shortcut" data-id="${x.id}" aria-label="바로가기 삭제">×</button></div>${safeUrl?`<a href="${esc(safeUrl)}"><span class="shortcut-icon">${isGrab?'<img src="./src/img/grab.png" alt="Grab">':esc(x.icon||'↗')}</span><span><strong>${esc(x.name)}</strong><small>${isGrab?'그랩 앱 바로 열기':'앱 또는 웹 열기'}</small></span></a>`:`<div class="shortcut-invalid"><span class="shortcut-icon">⚠️</span><span><strong>${esc(x.name)}</strong><small>허용되지 않는 링크</small></span></div>`}</article>`)})}
