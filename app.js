...
function renderAuthState(){const logged=Boolean(authSession?.access_token);$('#authBtn').textContent=logged?'✓ 관리자 로그인됨':'🔐 관리자 로그인';$('#logoutBtn').hidden=!logged;$('#authMessage').textContent=logged?`${authSession.user?.email||'관리자'} 계정으로 연결됨`:'';renderAdminButton()}
function renderAdminButton(){const button=$('#manageAdminsBtn');if(!button)return;button.hidden=readOnly||!authSession||data?.liveShare?.isOwner===false}
...
function normalizeWebUrl(value){const url=String(value||'').trim();if(!url)return '';if(/^https?:\/\//i.test(url))return url;if(/^[a-z][a-z0-9+.-]*:/i.test(url))return '';return 'https://'+url.replace(/^\/+/, '')}
function linkifyText(value){const text=String(value||'');let html='',last=0;for(const match of text.matchAll(/https?:\/\/[^\s<>"']+/gi)){let url=match[0],tail='';while(/[.,!?;:)\]]$/.test(url)){tail=url.slice(-1)+tail;url=url.slice(0,-1)}html+=esc(text.slice(last,match.index))+'<a href="'+esc(url)+'" target="_blank" rel="noopener noreferrer">'+esc(url)+'</a>'+esc(tail);last=match.index+match[0].length}return html+esc(text.slice(last))}
function renderShortcuts(){const el=$('#shortcutList');el.innerHTML='';const items=data.shortcuts||[];if(!items.length)return empty(el,'여행 중 자주 사용하는 앱이나 사이트를 등록해 보세요.');items.forEach(x=>{const appName=String(x.name||'');const isAndroid=/Android/i.test(navigator.userAgent);const isGrab=/그랩|grab/i.test(appName);const isPapago=/파파고|papago|번역기/i.test(appName);const isGoogleMaps=/구글\s*지도|google\s*maps?|google지도/i.test(appName);const isCalculator=/계산기|calculator/i.test(appName);let directUrl='';let directLabel='앱 또는 웹 열기';if(isAndroid&&isGrab){directUrl='intent://open?screenType=BOOKING#Intent;scheme=grab;package=com.grabtaxi.passenger;action=android.intent.action.VIEW;category=android.intent.category.BROWSABLE;end';directLabel='그랩 앱 바로 열기'}else if(isAndroid&&isPapago){directUrl='android-app://com.naver.labs.translator';directLabel='Papago 앱 바로 열기'}else if(isAndroid&&isGoogleMaps){directUrl='intent://0,0?q=#Intent;scheme=geo;package=com.google.android.apps.maps;action=android.intent.action.VIEW;category=android.intent.category.BROWSABLE;end';directLabel='Google 지도 바로 열기'}else if(isAndroid&&isCalculator){directUrl='#calculator';directLabel='앱 계산기 열기'}const safeUrl=directUrl||safeShortcutUrl(x.url);const autoIcon=isCalculator?'🧮':isPapago?'🦜':isGoogleMaps?'🗺️':'';el.insertAdjacentHTML('beforeend',`<article class="shortcut-card"><div class="shortcut-actions"><button class="edit" data-edit="shortcut" data-id="${x.id}" aria-label="바로가기 수정">✎</button><button class="delete" data-del="shortcut" data-id="${x.id}" aria-label="바로가기 삭제">×</button></div>${safeUrl?`<a href="${esc(safeUrl)}" ${isCalculator?'data-open-calculator':''}><span class="shortcut-icon">${isGrab?'<img src="./src/img/grab.png" alt="Grab">':esc(autoIcon||x.icon||'↗')}</span><span><strong>${esc(x.name)}</strong><small>${directLabel}</small></span></a>`:`<div class="shortcut-invalid"><span class="shortcut-icon">⚠️</span><span><strong>${esc(x.name)}</strong><small>허용되지 않는 링크</small></span></div>`}</article>`)})}
...
function renderBoard(){const el=$('#boardList');el.innerHTML='';const items=[...data.board].sort((a,b)=>b.id-a.id);if(!items.length)return empty(el,'여행에 참고할 정보를 기록해 보세요.');items.forEach(x=>{const safeUrl=normalizeWebUrl(x.url);el.insertAdjacentHTML('beforeend',`<article class="board-card"><div class="board-actions"><button class="edit" data-edit="board" data-id="${x.id}" aria-label="정보 수정">✎</button><button class="delete" data-del="board" data-id="${x.id}" aria-label="정보 삭제">×</button></div><h3>${esc(x.title)}</h3><p>${linkifyText(x.content)}</p>${safeUrl?`<a href="${esc(safeUrl)}" target="_blank" rel="noopener">관련 링크 열기 ↗</a>`:''}</article>`)})}
...
async function connectAccountData(){
  ...
  if(!rows.length){
    for(const plan of localPlans){
      ...
      plan.liveShare={id:row.id,viewerToken:row.viewer_token,isOwner:true};
    }
...
async function renderAdminList(){const list=$('#adminList');list.innerHTML='<p class="admin-loading">목록을 불러오는 중...</p>';const rows=await authRpc('list_trip_admins',{p_id:data.liveShare.id});if(!rows?.length)return empty(list,'아직 초대한 공동 관리자가 없어요.');list.innerHTML=rows.map(row=>'<div class="admin-row"><div><strong>'+esc(row.admin_email)+'</strong><small>'+new Date(row.created_at).toLocaleDateString('ko-KR')+' 초대</small></div><button type="button" class="delete-admin" data-admin-email="'+esc(row.admin_email)+'">해제</button></div>').join('')}
$('#manageAdminsBtn').onclick=async()=>{try{const meta=await ensureLiveShare();if(meta.isOwner===false)throw new Error('원소유자만 공동 관리자를 관리할 수 있습니다.');$('#adminTripName').textContent=data.trip.title+' 여행의 공동 관리자를 초대하거나 해제할 수 있어요.';$('#adminMessage').textContent='';$('#adminModal').showModal();await renderAdminList()}catch(error){alert(error.message||'공동 관리자 화면을 열지 못했습니다.')}};
$('#closeAdmins').onclick=()=>$('#adminModal').close();
$('#adminInviteForm').onsubmit=async e=>{e.preventDefault();const email=$('#adminEmail').value.trim().toLowerCase();const message=$('#adminMessage');try{message.textContent='초대하는 중...';const added=await authRpc('invite_trip_admin',{p_id:data.liveShare.id,p_email:email});if(!added)throw new Error('초대하지 못했습니다.');e.target.reset();message.textContent=email+' 님을 공동 관리자로 초대했습니다. 해당 이메일로 로그인하면 이 여행이 보여요.';await renderAdminList()}catch(error){message.textContent=error.message||'공동 관리자를 초대하지 못했습니다.'}};
$('#adminList').onclick=async e=>{const button=e.target.closest('[data-admin-email]');if(!button)return;const email=button.dataset.adminEmail;if(!confirm(email+' 님의 공동 관리자 권한을 해제할까요?'))return;try{button.disabled=true;const removed=await authRpc('remove_trip_admin',{p_id:data.liveShare.id,p_email:email});if(!removed)throw new Error('권한을 해제하지 못했습니다.');$('#adminMessage').textContent=email+' 님의 권한을 해제했습니다.';await renderAdminList()}catch(error){$('#adminMessage').textContent=error.message||'권한을 해제하지 못했습니다.';button.disabled=false}};
...
$('#voucherBatchInput').onchange=async e=>{const files=[...e.target.files];if(!files.length)return;const matched=[],unmatched=[];for(const file of files){const bookingId=file.name.match(/(?<!\d)\d{10}(?!\d)/)?.[0];const stay=bookingId&&(data.accommodations||[]).find(item=>String(item.booking||'').replace(/\D/g,'')===bookingId);if(!stay){unmatched.push(file.name);continue}try{stay.voucherId=await saveVoucher(file,stay.voucherId);stay.voucherName=file.name;matched.push(`${bookingId} → ${stay.name}`)}catch{unmatched.push(file.name)}}if(matched.length)persist();const message=[matched.length?`연결 완료 ${matched.length}개\n${matched.join('\n')}`:'연결된 PDF가 없습니다.',unmatched.length?`\n일치 숙소 없음 ${unmatched.length}개\n${unmatched.join('\n')}`:''].join('');alert(message);e.target.value=''};
...
$('#modalForm').onsubmit=async e=>{
  ...
  if(mode==='trip')data.trip={...obj,people:Number(obj.people),budget:Number(obj.budget||0)};
    ...
    if(mode==='board'){const item=data.board.find(x=>x.id===editing.id);Object.assign(item,obj)}
...
let calcDisplay='0',calcFirst=null,calcOperator=null,calcReset=false;
function updateCalculator(){if(calcDisplay==='오류')return $('#calculatorDisplay').textContent=calcDisplay;const [integer,decimal]=String(calcDisplay).split('.');const formatted=Number(integer||0).toLocaleString('ko-KR');$('#calculatorDisplay').textContent=decimal!==undefined?formatted+'.'+decimal:formatted}
function calculatePair(a,b,op){if(op==='+')return a+b;if(op==='-')return a-b;if(op==='*')return a*b;if(op==='/')return b===0?NaN:a/b;return b}
document.addEventListener('click',e=>{const launcher=e.target.closest('[data-open-calculator]');if(launcher){e.preventDefault();$('#calculatorModal').showModal();updateCalculator();loadPhpRate();return}const key=e.target.closest('[data-calc]');if(!key)return;const value=key.dataset.calc;if(/^\d$/.test(value)){calcDisplay=calcDisplay==='0'||calcReset?value:calcDisplay+value;calcReset=false}else if(value==='.'&&!calcDisplay.includes('.')){calcDisplay+=(calcReset?'0': '')+'.';calcReset=false}else if(value==='clear'){calcDisplay='0';calcFirst=null;calcOperator=null;calcReset=false}else if(value==='sign'){calcDisplay=String(-Number(calcDisplay))}else if(value==='percent'){calcDisplay=String(Number(calcDisplay)/100)}else if(value==='='){if(calcOperator&&calcFirst!==null){const result=calculatePair(calcFirst,Number(calcDisplay),calcOperator);calcDisplay=Number.isFinite(result)?String(Number(result.toFixed(10))):'오류';calcFirst=null;calcOperator=null;calcReset=true}}else{if(calcOperator&&calcFirst!==null&&!calcReset)calcFirst=calculatePair(calcFirst,Number(calcDisplay),calcOperator);else calcFirst=Number(calcDisplay);calcOperator=value;calcReset=true}updateCalculator()});
$('#closeCalculator').onclick=()=>$('#calculatorModal').close();
const PHP_RATE_KEY='travelGoPhpKrwRate';
let phpRate=24;
function renderPhpRate(status=''){ $('#phpKrwRate').value=phpRate.toFixed(2);$('#phpRateStatus').textContent=status||`1 PHP ≈ ${phpRate.toFixed(2)} KRW · 실제 환전 금액은 수수료에 따라 달라질 수 있어요.` }
async function loadPhpRate(force=false){try{const cached=JSON.parse(localStorage.getItem(PHP_RATE_KEY)||'null');if(!force&&cached?.rate&&Date.now()-cached.savedAt<86400000){phpRate=Number(cached.rate);renderPhpRate(`${cached.date||'최근'} 기준 저장 환율 · 1 PHP ≈ ${phpRate.toFixed(2)} KRW`);return}$('#phpRateStatus').textContent='최신 환율을 불러오는 중…';const response=await fetch('https://api.frankfurter.dev/v2/rate/PHP/KRW');if(!response.ok)throw new Error('rate');const result=await response.json();phpRate=Number(result.rate);if(!Number.isFinite(phpRate))throw new Error('rate');localStorage.setItem(PHP_RATE_KEY,JSON.stringify({rate:phpRate,date:result.date,savedAt:Date.now()}));renderPhpRate(`${result.date} 기준 · 1 PHP ≈ ${phpRate.toFixed(2)} KRW`)}catch{renderPhpRate('환율 자동 조회 실패 · 표시된 환율을 직접 수정해 주세요.')}}
$('#refreshPhpRate').onclick=()=>loadPhpRate(true);
$('#phpKrwRate').oninput=e=>{const value=Number(e.target.value);if(value>0){phpRate=value;$('#phpRateStatus').textContent='직접 입력한 환율을 사용 중입니다.';$('#phpAmount').dispatchEvent(new Event('input'))}};
function parseFxNumber(value){return Number(String(value||'').replace(/,/g,''))}
function formatFxInput(value){const raw=String(value||'').replace(/,/g,'').replace(/[^0-9.]/g,'');if(!raw)return '';const [integer,decimal]=raw.split('.');const formatted=Number(integer||0).toLocaleString('ko-KR');return raw.includes('.')?formatted+'.'+(decimal||''):formatted}
$('#phpAmount').oninput=e=>{e.target.value=formatFxInput(e.target.value);const value=parseFxNumber(e.target.value);$('#krwAmount').value=e.target.value===''?'':Math.round(value*phpRate).toLocaleString('ko-KR')};
$('#krwAmount').oninput=e=>{e.target.value=formatFxInput(e.target.value);const value=parseFxNumber(e.target.value);$('#phpAmount').value=e.target.value===''?'':Number((value/phpRate).toFixed(2)).toLocaleString('ko-KR',{maximumFractionDigits:2})};window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();installPrompt=e;$('#installBtn').hidden=false});
